...
(function($){
    ...
    var settings = $.extend({
			...
			tpl: {
				...
				highlighter:	'<div id="feedback-highlighter"><div class="feedback-logo">Feedback</div><p>Click and drag on the page to help us better understand your feedback. You can move this dialog if it\'s in the way.</p><button class="feedback-sethighlight hidden feedback-active"></button><div class="feedback-buttons"><button id="feedback-highlighter-next" class="feedback-next-btn feedback-btn-gray">Next</button><button id="feedback-highlighter-back" class="feedback-back-btn feedback-btn-gray">Back</button></div><div class="feedback-wizard-close"></div></div>',
				overview:		'<div id="feedback-overview"><div class="feedback-logo">Feedback</div><div id="feedback-overview-description"><div id="feedback-overview-description-text"><h3>Description</h3></div></div><div id="feedback-overview-screenshot"><h3>Screenshot</h3></div><div class="feedback-buttons"><button id="feedback-submit" class="feedback-submit-btn feedback-btn-blue">Submit</button><button id="feedback-overview-back" class="feedback-back-btn feedback-btn-gray">Back</button></div><div id="feedback-overview-error">Please enter a description.</div><div id="feedback-overview-email-error">Please enter a email.</div><div id="feedback-overview-email-error-valid">Invalid email address.</div><div class="feedback-wizard-close"></div></div>',
					...
					html2canvas($('body'), {
						onrendered: function(canvas) {
							...
							if(settings.showDescriptionModal) {
								...
								$('#feedback-overview-description-text>h3.email').remove();
...
if(typeof $('#request-email').val() == 'undefined'){
email = '<h3 class="email">Enter Email to which we can contact you</h3><input id="feedback-overview-email" value="">'
}
else{
email = '<h3 class="email hidden">Enter Email to which we can contact you</h3><input class="hidden" id="feedback-overview-email" value="'+ $('#request-email').val() +'">'
}
$(email).insertAfter('#feedback-overview-description-text textarea:eq(0)');
				...
				$(document).on('click', '#feedback-overview-back', function(e) {
					...
					$('#feedback-overview-email-error').hide();
					$('#feedback-overview-email-error-valid').hide();
				});
				...
				$(document).on('click', '#feedback-submit', function() {
					...
					if ($('#feedback-note').val().length > 0 && $('#feedback-overview-email').val().length > 0) {

				var expr = /^([\w-\.]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([\w-]+\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\]?)$/;
if(expr.test($('#feedback-overview-email').val())){
post.img = img;
						post.note = $('#feedback-note').val();
						post.feedbackemail = $('#feedback-overview-email').val();
						...
						$('#feedback-overview-email-error-valid').show();
}
...
if(typeof $('#request-email').val() == 'undefined'){
if ($('#feedback-overview-email').val().length == 0){
						$('#feedback-overview-email-error').show();}
					}
}
				});
			});
